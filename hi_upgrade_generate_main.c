...
HI_S32 main(HI_S32 argc, HI_CHAR** argv)
{
    ...
    if(pktType == UPGRADE_IMAGES_BY_UBOOT) {
        int ret = snprintf(szReleasePacketName, HI_UPGRADE_PKT_PATH_MAX_LEN, "./"HI_UPGRADE_PKT_PREFIX"_%s_%s."UPGRADE_PKT_SUFFIX,
                 szModel, szSoftVersion);
        if (ret < 0) {
            abort();
        }                 
    } else {
        int ret = snprintf(szReleasePacketName, HI_UPGRADE_PKT_PATH_MAX_LEN, "./"HI_UPGRADE_PKT_PREFIX"_%s_%s."UPGRADE_PKT_APP_SUFFIX,
                 szModel, szSoftVersion);
        if (ret < 0) {
            abort();
        }
